...
interface ILinkProviderProps {
  onLink(link: ILink, duration: number): void;
  links: ILink[];
  minTimeslot?: number;
  timeslot?: number;
}
...
export default class LinkProvider extends PureComponent<ILinkProviderProps> {
  ...
  _run = (index: number, links: ILink[]) => {
    this.props.onLink(links[index], this._timeouts[index]);
  ...
  _calculateTimeslots = (links: ILink[]) => {
    ...
    this._timeouts = calculateTimeSlots(links.map(({ score }) => score), this.props.timeslot!, this.props.minTimeslot!);
